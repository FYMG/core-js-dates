...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    Math.floor((Date.parse(dateEnd) - Date.parse(dateStart)) / 86400000) + 1
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) result += 1;
  }
...
function getWeekNumberByDate(date) {
  ...
  const diffStart = new Date(firstDay);
  diffStart.setDate(firstDay.getDate() + ((0 - firstDay.getDay() + 7) % 7));
  const diffEnd = new Date(date);
  diffEnd.setDate(date.getDate() + ((0 - date.getDay() + 7) % 7));
  const diffDays = Math.floor((diffEnd - diffStart) / (1000 * 60 * 60 * 24));
...
function getNextFridayThe13th(date) {
  ...
  while (true) {
    if (nextDate.getDay() === 5 && nextDate.getDate() === 13) return nextDate;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const parseDate = (str) =>
    new Date(Date.parse(str.split('-').reverse().join('/')));
  ...
  while (currentDate <= endDate) {
    ...
    if (state === states.working) {
      schedule.push(currentDate.toLocaleDateString('es-CL'));
      if (count === countWorkDays) {
        state = states.off;
        count = 0;
      }
    } else if (count === countOffDays) {
